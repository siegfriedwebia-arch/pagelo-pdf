...
(function () {
  ...
  function renderPreview() {
    ...
    if (!name) { previewBox.innerHTML = '<p class="hint">Elige al menos una hoja.</p>'; return; }
    ...
    previewBox.innerHTML = `<p class="hint">Vista previa de «${name}» (primeras filas):</p>`;
  ...
  U.$("#file").addEventListener("change", async e => {
    ...
    try {
      ...
      wb.SheetNames.forEach((n, i) => {
        ...
        l.querySelector("span").textContent = `${n} (${rows} filas)`;
        sheetsBox.appendChild(l);
      });
      if (!U.$$("input[name=sheet]:checked").length) throw new P.FriendlyError("El archivo no tiene datos.");
      U.$("#doc-info").textContent = file.name;
      P.showWork();
      renderPreview();
      btn.disabled = false;
    } catch (err) {
      P.fail(status, err instanceof P.FriendlyError ? err : new P.FriendlyError("No se ha podido leer el archivo. Comprueba que es una hoja de cálculo (.xlsx, .xls, .ods o .csv) y que no tiene contraseña."));
    }
  });
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (!names.length) throw new P.FriendlyError("Elige al menos una hoja.");
