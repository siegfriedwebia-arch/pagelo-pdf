  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      ...
      for (let k = 0; k < pages.length; k++) {
        status.textContent = `Convirtiendo página ${pages[k]} (${k + 1} de ${pages.length})…`;
        ...
        files.push({ name: `${name}-pagina-${pages[k]}.${ext}`, data: blob });
      }
      if (files.length === 1) {
        P.finish({ title: "Imagen lista", detail: U.formatBytes(files[0].data.size), blob: files[0].data, name: files[0].name });
      } else {
        status.textContent = "Creando el ZIP…";
        const zip = await P.zip(files);
        P.finish({ title: "Imágenes listas", detail: `${files.length} imágenes ${ext.toUpperCase()} en un ZIP · ${U.formatBytes(zip.size)}`, blob: zip, name: `${name}-imagenes.zip` });
      }
    } catch (e) {
      P.fail(status, e);
      btn.disabled = false;
