...
(function () {
  ...
  async function editable() {
    ...
    for (let i = 1; i <= view.numPages; i++) {
      status.textContent = `Reading page ${i} of ${view.numPages}…`;
    ...
    if (!sizes.length) throw new P.FriendlyError("This PDF has no text (it looks scanned). Use the “Exact layout” option.");
    sizes.sort((a, b) => a - b);
  ...
  async function asImages() {
    ...
    const sections = [];
    for (let i = 1; i <= view.numPages; i++) {
      status.textContent = `Converting page ${i} of ${view.numPages}…`;
      bar.style.width = Math.round((i / view.numPages) * 90) + "%";
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      ...
      P.finish({
        ...
        detail: `${U.formatBytes(blob.size)} · Open it with Word, Google Docs or LibreOffice.`,
        blob,
