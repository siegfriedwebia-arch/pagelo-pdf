...
(function () {
  ...
  async function add(fileList) {
    ...
    await ready;
    if (!pdfs.length) { status.className = "status error"; status.textContent = "Choose PDF files."; return; }
  ...
  function draw() {
    ...
    items.forEach((it, i) => {
      ...
      li.innerHTML = `
        ...
        <div class="mini"></div>
        <div class="meta"><b></b><span>${it.pages} ${U.pl(it.pages, "page", "pages")} · ${U.formatBytes(it.file.size)}</span></div>
        <div class="act">
          <button type="button" data-a="up" aria-label="Move up">↑</button>
          <button type="button" data-a="down" aria-label="Move down">↓</button>
          <button type="button" data-a="del" aria-label="Remove">✕</button>
        </div>`;
    ...
    status.textContent = items.length < 2
      ? "Add at least one more PDF to merge them."
      : `${items.length} files · ${total} pages in total. Reorder them by dragging or with the arrows.`;
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      ...
      for (let i = 0; i < items.length; i++) {
        status.textContent = `Merging ${i + 1} of ${items.length}…`;
      ...
      P.finish({
        title: "PDF merged",
        detail: `${out.getPageCount()} pages · ${U.formatBytes(bytes.length)}`,
        blob: P.blob(bytes),
        name: "merged.pdf"
      });
    } catch (e) {
