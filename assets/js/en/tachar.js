...
(function () {
  ...
  async function showPage(n) {
    ...
    U.$("#page-label").textContent = `Page ${n} of ${view.numPages}`;
    U.$("#prev").disabled = n <= 1;
  ...
  function update() {
    ...
    status.textContent = boxes.length
      ? `${boxes.length} ${U.pl(boxes.length, "area", "areas")} ${U.pl(boxes.length, "redacted", "redacted")} on ${pages} ${U.pl(pages, "page", "pages")}. Tap an area to remove it.`
      : "Drag across the page to redact, or find data automatically.";
  }
  ...
  U.$("#find").addEventListener("click", async () => {
    ...
    if (!regexes.length) { status.textContent = "Tick which data to look for or type some text."; return; }
    status.textContent = "Searching…";
    const n = await search(regexes);
    await showPage(current);
    update();
    const msg = n ? `Redacted ${n} matches. Check every page before saving.` : "Nothing found. If the PDF is scanned, redact by hand by dragging across the page.";
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      ...
      for (let n = 1; n <= view.numPages; n++) {
        status.textContent = `Processing page ${n} of ${view.numPages}…`;
      ...
      P.finish({
        title: "Data redacted",
        detail: `${U.formatBytes(res.length)} · Redacted pages are now images: the hidden text can't be recovered.`,
        blob: P.blob(res),
        name: U.baseName(file.name) + "-redacted.pdf"
      });
