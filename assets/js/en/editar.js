...
(function () {
  ...
  function draw(it) {
    ...
    el.insertAdjacentHTML("beforeend", '<button class="del" type="button" aria-label="Remove">✕</button>' + (it.type === "text" ? "" : '<button class="size" type="button" aria-label="Resize">⤡</button>'));
  ...
  U.$("#add-text").addEventListener("click", () => add({ type: "text", text: "Type here", size: 14, color: "#111111", fx: 0.1, fy: 0.1 }));
  ...
  function update() {
    ...
    status.textContent = items.length
      ? `${items.length} ${U.pl(items.length, "change", "changes")}. Tap an item to edit or move it.`
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      ...
      P.finish({ title: "PDF edited", detail: U.formatBytes(out.length), blob: P.blob(out), name: U.baseName(file.name) + "-edited.pdf" });
