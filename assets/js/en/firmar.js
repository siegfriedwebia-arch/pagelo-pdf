...
(function () {
  ...
  function setSignature(url, ratio) {
    ...
    prev.innerHTML = url ? `<img alt="Your signature" src="${url}">` : '<span class="hint">Your signature will appear here</span>';
  ...
  async function showPage(n) {
    ...
    pageLabel.textContent = `Page ${n} of ${view.numPages}`;
    U.$("#prev").disabled = n <= 1;
  ...
  function addBox(pl) {
    ...
    box.innerHTML = `<img alt=""><button class="del" type="button" aria-label="Remove">✕</button><button class="size" type="button" aria-label="Resize">⤡</button>`;
  ...
  function update() {
    ...
    status.textContent = placements.length
      ? `${placements.length} ${U.pl(placements.length, "item", "items")} ${U.pl(placements.length, "placed", "placed")}. Drag them to move them and use ⤡ to resize.`
      : "Create your signature and click “Place on this page”.";
  }
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      ...
      P.finish({ title: "PDF signed", detail: U.formatBytes(out.length), blob: P.blob(out), name: U.baseName(file.name) + "-signed.pdf" });
    } catch (e) {
      P.fail(status, e);
      btn.disabled = false;
    }
