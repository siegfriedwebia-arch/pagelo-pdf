...
(function () {
  ...
  function refresh() {
    ...
    U.$$(".pg figcaption", grid).forEach((c, i) => { c.textContent = `Page ${i + 1}`; });
    status.className = "status";
    status.textContent = n ? `${n} ${U.pl(n, "page", "pages")}. Drag with ⠿ to change the order.` : "There are no pages left.";
    btn.disabled = n === 0;
  }
  ...
  function card(page) {
    ...
    fig.innerHTML = `<div class="thumb"></div><figcaption></figcaption>
      <div class="org-tools">
        <button type="button" class="org-drag" aria-label="Drag to move">⠿</button>
        <button type="button" data-a="left" aria-label="Move left">‹</button>
        <button type="button" data-a="right" aria-label="Move right">›</button>
        <button type="button" data-a="rot" aria-label="Rotate">↻</button>
        <button type="button" data-a="del" aria-label="Delete">✕</button>
      </div>`;
    const thumb = fig.querySelector(".thumb");
    if (page.blank) thumb.innerHTML = '<span class="hint">Blank</span>';
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      ...
      P.finish({ title: "PDF organized", detail: `${out.getPageCount()} pages · ${U.formatBytes(bytes.length)}`, blob: P.blob(bytes), name: `${name}-organized.pdf` });
