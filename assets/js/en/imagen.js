...
(function () {
  ...
  input.addEventListener("change", () => {
    ...
    status.textContent = files.length ? `${files.length} image(s) ready.` : "Select images.";
  });
  ...
  btn.addEventListener("click", async () => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      status.textContent = `Processing ${i + 1} of ${files.length}…`;
      try {
        ...
        span.innerHTML = `${U.formatBytes(files[i].size)} → <strong>${U.formatBytes(out.blob.size)}</strong> ` +
          ...
          ` · ${out.w}×${out.h}` + (out.keptOriginal ? " · already optimized" : "");
        ...
        span.textContent = "Your browser can't read this format.";
      }
    }
    const pct = before ? Math.round((1 - after / before) * 100) : 0;
    status.textContent = mode === "comprimir"
      ? `Done. Total: ${U.formatBytes(before)} → ${U.formatBytes(after)}${pct > 0 ? ` (−${pct}%)` : ""}.`
      : `Done. ${outputs.length} image(s) ${mode === "convertir" ? "converted" : "resized"}.`;
  ...
  zipBtn.addEventListener("click", async () => {
    ...
    U.download(blob, `images-${{ comprimir: "compressed", convertir: "converted", redimensionar: "resized" }[mode]}.zip`);
