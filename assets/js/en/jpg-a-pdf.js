...
(function () {
  ...
  function add(fileList) {
    ...
    if (!imgs.length) { status.className = "status error"; status.textContent = "Choose JPG, PNG or WebP images."; return; }
  ...
  function draw() {
    ...
    items.forEach((it, i) => {
      ...
      li.innerHTML = `<span class="handle" aria-hidden="true">⋮⋮</span><div class="mini"><img alt=""></div>
        ...
        <div class="act"><button type="button" data-a="up" aria-label="Move up">↑</button><button type="button" data-a="down" aria-label="Move down">↓</button><button type="button" data-a="del" aria-label="Remove">✕</button></div>`;
      li.querySelector("img").src = it.url;
      li.querySelector("b").textContent = it.file.name;
      list.appendChild(li);
    });
    status.className = "status";
    status.textContent = items.length ? `${items.length} ${U.pl(items.length, "image", "images")}. Each one will be a page.` : "Add images.";
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        catch (e) { throw new P.FriendlyError(`Can't read “${items[i].file.name}” in this browser.`); }
        let pw, ph;
      ...
      P.finish({ title: "PDF created", detail: `${items.length} ${U.pl(items.length, "page", "pages")} · ${U.formatBytes(bytes.length)}`, blob: P.blob(bytes), name: "images.pdf" });
    } catch (e) {
      P.fail(status, e);
      btn.disabled = false;
    }
