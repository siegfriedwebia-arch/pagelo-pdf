...
(function () {
    ...
    try {
      ...
      if (!form.getFields().length) {
        throw new P.FriendlyError(hasXfa
          ? "This form uses a special format (XFA) that only works in Adobe Acrobat Reader. Open it there to fill it in."
          : "This PDF has no fields to fill in. Use “Edit PDF” to type on top of the document.");
      }
      U.$("#doc-info").textContent = `${file.name} · ${view.numPages} pages`;
      P.showWork();
      const r = await build();
      status.className = "status";
      status.textContent = `${controls.length} fields found. Fill them in right on the document.`;
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      ...
      P.finish({ title: "Form filled in", detail: U.formatBytes(out.length), blob: P.blob(out), name: U.baseName(file.name) + "-filled.pdf" });
