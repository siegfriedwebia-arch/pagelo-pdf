...
(function () {
  ...
  async function addFiles(list) {
    ...
    for (const f of files) {
      ...
      } catch (e) { U.toast(`Can't read ${f.name}`); }
    }
    if (!shots.length) { P.fail(status, new P.FriendlyError("None of the photos could be read.")); return; }
    P.showWork();
    drawShots();
  ...
  function drawShots() {
    ...
    shots.forEach((s, i) => {
      ...
      img.alt = `Page ${i + 1}`;
      d.appendChild(img);
      d.insertAdjacentHTML("beforeend", `P. ${i + 1}`);
  ...
  const handles = [0, 1, 2, 3].map(k => {
    ...
    h.className = "corner";
    h.setAttribute("aria-label", "Corner " + (k + 1));
    wrap.appendChild(h);
    let drag = false;
  ...
  U.$("#detect").addEventListener("click", () => {
    ...
    else U.toast("The paper can't be detected. Adjust the corners by hand.");
  });
  ...
  U.$("#apply-all").addEventListener("click", () => { shots.forEach(s => { s.filter = shots[cur].filter; }); U.toast("Enhancement applied to all pages"); });

  function update() {
    btn.disabled = !shots.length;
    status.className = "status";
    status.textContent = `${shots.length} ${U.pl(shots.length, "page", "pages")}. Adjust the corners to the edges of the paper.`;
  }
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      ...
      for (let i = 0; i < shots.length; i++) {
        status.textContent = `Creating page ${i + 1} of ${shots.length}…`;
        await U.sleep(20);
      ...
      P.finish({ title: "Document scanned", detail: `${shots.length} ${U.pl(shots.length, "page", "pages")} · ${U.formatBytes(out.length)}`, blob: P.blob(out), name: "scan.pdf" });
