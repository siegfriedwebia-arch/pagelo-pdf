...
(function () {
  ...
  function preview() {
    ...
    try {
      const p = plan();
      status.textContent = `Will create ${p.length} ${U.pl(p.length, "file", "files")}: ` +
        p.slice(0, 6).map(([a, b]) => a === b ? `p. ${a}` : `pp. ${a}-${b}`).join(" · ") + (p.length > 6 ? " …" : "");
      btn.disabled = false;
    } catch (e) {
  ...
  U.$("#file").addEventListener("change", async e => {
    ...
    try {
      ...
      total = view.numPages;
      U.$("#doc-info").textContent = `${f.name} · ${total} pages`;
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      ...
      for (let i = 0; i < parts.length; i++) {
        status.textContent = `Creating file ${i + 1} of ${parts.length}…`;
        const [a, b] = parts[i];
      ...
      if (files.length === 1) {
        P.finish({ title: "PDF split", detail: U.formatBytes(files[0].data.length), blob: P.blob(files[0].data), name: files[0].name });
      } else {
        status.textContent = "Creating the ZIP…";
        const zip = await P.zip(files);
        P.finish({ title: "PDF split", detail: `${files.length} PDF files in a ZIP · ${U.formatBytes(zip.size)}`, blob: zip, name: `${name}-split.zip` });
      }
    } catch (e) {
      P.fail(status, e);
      btn.disabled = false;
