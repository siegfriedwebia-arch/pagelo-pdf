...
(function () {
  ...
  function label(o, n, count) {
    return { n: `${n}`, pag: `Page ${n}`, pagde: `Page ${n} of ${count}`, slash: `${n} / ${count}` }[o.fmt];
  }
  ...
  async function preview() {
    ...
    try {
      ...
      status.textContent = `Preview of page ${Math.min(opts().from, total)}.`;
      btn.disabled = false;
  ...
  U.$("#file").addEventListener("change", async e => {
    ...
    try {
      ...
      U.$("#doc-info").textContent = `${file.name} · ${total} pages`;
      U.$("#from").max = total;
      P.showWork();
      preview();
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      ...
      P.finish({ title: "Pages numbered", detail: U.formatBytes(out.length), blob: P.blob(out), name: U.baseName(file.name) + "-numbered.pdf" });
