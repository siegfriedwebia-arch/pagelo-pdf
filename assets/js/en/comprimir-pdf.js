...
(function () {
  ...
  U.$("#file").addEventListener("change", async e => {
    ...
    try {
      ...
      U.$("#doc-info").textContent = `${file.name} · ${view.numPages} pages · ${U.formatBytes(file.size)}`;
      P.showWork();
  ...
  async function raster(level) {
    ...
    for (let i = 1; i <= view.numPages; i++) {
      status.textContent = `Compressing page ${i} of ${view.numPages}…`;
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      ...
      const name = U.baseName(file.name) + "-compressed.pdf";
      P.finish({
        title: smaller ? "PDF compressed" : "This PDF was already optimized",
        detail: smaller
          ? P.sizeText(file.size, result.length)
          : `It couldn't be reduced (${U.formatBytes(file.size)}). Try a higher level or download the original.`,
