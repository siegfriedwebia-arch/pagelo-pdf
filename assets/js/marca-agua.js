...
(function () {
  ...
  async function build(onlyFirst) {
    ...
    if (type === "image") {
      ...
      catch (e) { throw new P.FriendlyError("El texto tiene caracteres no admitidos (por ejemplo, emojis). Usa letras, números y signos normales."); }
    }
  ...
  async function preview() {
    ...
    try {
      ...
      status.textContent = "Vista previa de la primera página.";
      btn.disabled = false;
  ...
  btn.addEventListener("click", async () => {
    btn.disabled = true;
    status.textContent = "Aplicando la marca de agua…";
    try {
      const out = await build(false);
      P.finish({ title: "Marca de agua añadida", detail: U.formatBytes(out.length), blob: P.blob(out), name: U.baseName(file.name) + "-marca-de-agua.pdf" });
